#*----------v Function preview-EAPUpdate() v----------
    function preview-EAPUpdate {
        ...
        AddedTwitter: URL
        REVISIONS
        * 1:11 PM 8/23/2021 fixed bug in regex ps replace; haven't ested balance of fancy substring replace options; %g, %s working.
        .DESCRIPTION
        preview-EAPUpdate.ps1 - Code to approximate EmailAddressTemplate-generated email addresses
        ...
        .EXAMPLE
        PS> preview-EAPUpdate  -addresstemplate '%3g%5s@contoso.com' -Recipient $trcp -verbose ;
        Preview target recipient using a template that takes givename[0-3]surname[0-5]@contoso.com
        .EXAMPLE
        PS> preview-EAPUpdate  -addresstemplate '%1g%s@microsoft.com' -Recipient $trcp -verbose ;
        Preview target recipient using %1g%s@microsoft.com|JWilson@microsoft.com|"[1stcharGivenName][surname]@domain"|
        .EXAMPLE
        PS> preview-EAPUpdate  -addresstemplate '%r._%d@microsoft.com' -Recipient $trcp -verbose ;
        Preview target recipient using %r._%d@microsoft.com|JamesC_Wilson@microsoft.com|"[replace periods in displayname with underscore]@domain"|
        .EXAMPLE
        ...
        PROCESS {
            ...
            $genAddr = $null ; 
            if($ptmpl -match '^@.*m$'){$genAddr = $ptmpl = $ptmpl.replace('@',"$($Recipient.alias)@")} ;
            if($ptmpl.contains('%g')){$genAddr = $ptmpl = $ptmpl.replace('%g',$usr.firstname)} ;
            if($ptmpl.contains('%s')){$genAddr = $ptmpl = $ptmpl.replace('%s',$usr.lastname)} ;
            if($ptmpl.contains('%d')){$genAddr = $ptmpl = $ptmpl.replace('%d',$usr.displayname)} ;
            if($ptmpl.contains('%i')){$genAddr = $ptmpl = $ptmpl.replace('%i',$usr.Initials)} ;
            if($ptmpl.contains('%m')){$genAddr = $ptmpl = $ptmpl.replace('%m',$Recipient.alias)} ;
            if($ptmpl -match '(%(\d)g)'){
                $ltrs = $matches[2] ; 
            ...
            if($ptmplZ -match "%r([a-zA-Z_0-9-._+&])([a-zA-Z_0-9-._+&])%(d|g|i|m|s)@.*$"){
                $x = $matches[1] ; 
                $y = $matches[2] ; 
                $vari = $matches[3] ; 
                write-host "Parsed:`nx:$($x)`ny:$($y)`nvari:$($vari)" ;
                switch ($vari){
                  'd' {
                      $ptmplZ = $ptmpl.replace("%d",$usr.displayname.replace($x,$y)) ;
                  }
                  'g'  {
                      $ptmplZ = $ptmpl.replace("%g",$usr.firstname.replace($x,$y)) ;
                  }
                  'i' {
                      $ptmplZ = $ptmpl.replace("%i",$usr.Initials.replace($x,$y)) ;
                  }
                  'm' {
                      $ptmplZ = $ptmpl.replace("%m",$Recipient.alias.replace($x,$y)) ;
                  }
                  's' {
                      $ptmplZ = $ptmpl.replace("%s",$usr.lastname.replace($x,$y)) ;
                  }
                  default {
        ...
        END {
            if($genAddr){$genAddr| write-output }
            else {
