...
Function Connect-ExchangeServerTDO {
        ...
        * 1:30 PM 9/5/2024 added  update-SecurityProtocolTDO() SB to begin
        * 12:49 PM 6/21/2024 flipped PSS Name to Exchange$($ExchVers[dd])
        * 12:57 PM 6/11/2024 Validated, Ex2010 & Ex2019, hub, mail & edge roles: tested â˜‘ï¸ on CMW mail role (Curly); and Jumpbox; 
            copied in CBH from repo copy, which has been updated/debugged compat on CMW Edge 
            includes local snapin detect & load for edge role (simplest EMS load option for Edge role, from David Paulson's original code; no longer published with Ex2010 compat)
        * 11:28 AM 5/30/2024 fixed failure to recognize existing functional PSSession; Made substantial update in logic, validate works fine with other orgs, and in our local orgs.
        * 4:02 PM 8/28/2023 debuged, updated CBH, renamed connect-ExchangeSErver -> Connect-ExchangeServerTDO (avoid name clashes, pretty common verb-noun combo).
...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        if($Msgs){
            ...
            $smsg = "EventID Distrib:" ;
            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
            else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
            $smsg = "`n#*------v EventID DISTRIB v------" ;
            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
            ...
            <# ---
            write-host -foregroundcolor green "# BAD ADDRESS FAILS: (EventID:'FAIL' & Source:'ROUTING'):`t$(($msgs|?{$_.EventID -eq 'FAIL' -AND $_.Source -eq 'ROUTING'}|measure).count)" ;
            write-host -foregroundcolor green "# REJECTED BY RECIPIENT SERVER : (EventID:'FAIL' & Source:'SMTP'):`t$(($msgs|?{$_.EventID -eq 'FAIL' -AND $_.Source -eq 'SMTP'}|measure).count)" ;
            write-host -foregroundcolor green "# BAD DOMAIN : (EventID:'FAIL' & Source:'DNS'):`t$(($msgs|?{$_.EventID -eq 'FAIL' -AND $_.Source -eq 'DNS'}|measure).count)" ;
            #---
            #>
            if($mDefers = $msgs|?{$_.EventID -eq 'DEFER'}){
                $smsg = "`n#*------v DEFER's Distribution v------" ;
                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
            ...
            if($mFails){
                $smsg = "`n#*------v FAIL's RecipientStatus Distribution v------" ;
                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                ...
                $smsg = "`n#*------^ FAIL's RecipientStatus Distribution ^------" ;
                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                if(($msgs|?{$_.EventID -eq 'FAIL'}).count -lt 20){
                    $smsg = "`n#*------v FAIL Details v------" ;
                    if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                    else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQAjkGGaBgHQSjCJORgU8273L
# pQigggI4MIICNDCCAaGgAwIBAgIQWsnStFUuSIVNR8uhNSlE6TAJBgUrDgMCHQUA
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRzPjpn
# +RxH+khyIHU8oQ9i5XeWSjANBgkqhkiG9w0BAQEFAASBgLLzUyxEfXVFlaZgnXl2
# SQ1Ep70X4rH1wXR5KMoic7UPPHKU8c+qHAX0xKsvyKy5j/U6IpFiKQtnOG428Joj
# 9eqVtJwq7dgxRKUyouM7bl1tpa1q+wdm7bbtNDEcmAaSQH/u7DmOqGo3OZbFpaBD
# hF7Px3amU9U+08wflF1SqMBr
